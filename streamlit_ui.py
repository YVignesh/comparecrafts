...
def build_filter_ui(df, label_prefix, saved_filters=None):
    ...
    for i in range(num_filters):
        ...
        col = st.selectbox(f"Filter {i+1} column ({label_prefix})", options=df.columns.tolist(), index=df.columns.get_loc(saved[0]) if saved else 0, key=f"{label_prefix}_col_{i}")
        op = st.selectbox(f"Condition for {col}", options=["==", "!=", ">", ">=", "<", "<=", "contains"], index=(["==", "!=", ">", ">=", "<", "<=", "contains"].index(saved[1]) if saved else 0), key=f"{label_prefix}_op_{i}")
...
def apply_filters(df, filters):
    for f in filters:
        ...
        try:
            if op == "==": df = df[df[col] == val_casted]
            elif op == "!=": df = df[df[col] != val_casted]
            elif op == ">": df = df[df[col] > val_casted]
            elif op == ">=": df = df[df[col] >= val_casted]
            elif op == "<": df = df[df[col] < val_casted]
            elif op == "<=": df = df[df[col] <= val_casted]
            elif op == "contains": df = df[df[col].astype(str).str.contains(str(val_casted), na=False, case=case, regex=regex)]
        except Exception as e:
...
if uploaded_files and len(uploaded_files) == 2:
    ...
    is_excel1, is_excel2 = file1.name.endswith(".xlsx"), file2.name.endswith(".xlsx")

    file_main = st.radio("Select Main File", [file1.name, file2.name], index=0)
    ...
    df_main = load_data(f_main, sheet_main)
    df_secondary = load_data(f_secondary, sheet_secondary)

    if df_main is not None and df_secondary is not None:
        filters_main = build_filter_ui(df_main, "Main", config_data.get("main_filters") if config_loaded else None)
        filters_secondary = build_filter_ui(df_secondary, "Secondary", config_data.get("secondary_filters") if config_loaded else None)
        ...
        columns_main = st.multiselect(f"Select columns from {file_main}", df_main.columns.tolist())
        columns_secondary = st.multiselect(f"Select columns from {file_secondary}", df_secondary.columns.tolist())
        ...
        if columns_main and columns_secondary and len(columns_main) == len(columns_secondary):
            ...
            for col1, col2 in zip(columns_main, columns_secondary):
                mapped = st.selectbox(f"Map '{col2}' to", options=columns_main, index=columns_main.index(col1), key=col2)
                column_mapping[col2] = mapped

            key_columns = st.multiselect("🔑 Select Key Columns (from Main file)", options=columns_main)
            if key_columns:
                ...
                def compare_df(df1, df2):
                    ...
                    result = []
                    for key in keys:
                        row1 = df1.loc[key] if key in df1.index else pd.Series([None]*len(columns), index=columns)
                        row2 = df2.loc[key] if key in df2.index else pd.Series([None]*len(columns), index=columns)
                        diff_row = {"Key": key}
                        changed = False
                        for col in columns:
                            ...
                            if pd.isna(val_old) != pd.isna(val_new) or str(val_old) != str(val_new):
                                changed = True
                        diff_row["ChangeType"] = "Added" if key not in df2.index else "Removed" if key not in df1.index else "Modified" if changed else "Unchanged"
                ...
                summary = diff_report['ChangeType'].value_counts().to_dict()
                st.json(summary)
                ...
                st.download_button("📥 Download Difference Report", data=to_excel_bytes(diff_report), file_name="difference_report.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")

                config_name = st.text_input("Enter config file name", value="comparison_config.json")
                comparison_config = {
                ...
                st.download_button("💾 Download Comparison Config", data=json.dumps(comparison_config, indent=2), file_name=config_name if config_name.endswith(".json") else config_name + ".json", mime="application/json")
        else:
